...
def make_bsp(n_dim):
    ...
    for q in range(n_dim):
        bsp_qc.rz(param.params[q],[q])
    return bsp_qc
def all_circuits_parallel(y_t, x_t, indices_list, n_dim, psix, psiy):
...
def run_svsm(n_dim,simulator, nb1, nb2):
    ...
    t0 = time.time()    
    circuit_train = all_circuits_parallel(data_train, data_train, indices_list_t, n_dim, psix,psiy)
